...
def get_callsign_info(callsign, include_address=True):
    ...
    if fcc_record:
        _type = (
            "Amateur Radio"
            if fcc_record["Service"] == "AMAT"
            else fcc_record["Service"]
        )
        _class = FCC_CLASS_CODES.get(fcc_record["Operator_Class"], "N/A")
        _zip = (
            fcc_record["Zip_Code"][0:5]
            if len(fcc_record["Zip_Code"]) > 5
            else fcc_record["Zip_Code"]
        )

        # Pretty-print the name
        name = (
            fcc_record["First_Name"]
            + " "
            + fcc_record["Middle_Initial"]
            + " "
            + fcc_record["Last_Name"]
        )
...
def fcc_callsign_lookup(callsign):
    ...
    Group_Code,
    Region_Code
FROM CallSignView WHERE Call_Sign = ?;""",
        (callsign,),
    )
    row = cursor.fetchone()
